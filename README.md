# Hi! 👋 My Name is GYEONGJIN JANG


<br>

## 🛠 기술 스택(SKill)
- Frontend: HTML, CSS, JavaScript, React(basic)
- Backend: Node.js, Python Flask
- Database: MySQL, DynamoDB
- Cloud/Infra: AWS Terraform(IaC)
- TeamWork : GitHub, Figma, Swagger

<br>

## 📚 프로젝트(Projects)
### 1. 스마트 화재 알리미 [링크](https://d2aftrh1mh1977.cloudfront.net/스마트화재알리미/index.html)
- 프로젝트 설명: Web Cam을 통해 화재를 감지하고 관리자에게 알림을 전송 화재에 대처하는 시스템
### 2. ImageBoard (이미지게시판) [링크](https://d2aftrh1mh1977.cloudfront.net/이미지게시판/index.html)
- 프로젝트 설명: 이미지 공유 및 커뮤니티 기능이 있는 게시판
### 3. 스마트홈 [링크](https://d2aftrh1mh1977.cloudfront.net/스마트홈/index.html)
- 프로젝트 설명: 집안에 전자기기를 자동제어 및 원격제어
### 4. 스마트팜 [링크](https://d2aftrh1mh1977.cloudfront.net/스마트팜/index.html)
- 프로젝트 설명: 작물이 성장할 수 있는 환경 조성을 자동화 
### 5. ArcadeGameHub (웹 게임 서비스) [링크](https://d2aftrh1mh1977.cloudfront.net/ArcadeGameHub/index.html)
- 프로젝트 설명: 모바일, 태블릿, 데스크탑 어디서나 이용할 수 있는 웹 게임 서비스
### 6. AWS 자격증 훈련 사이트 [링크](https://d2aftrh1mh1977.cloudfront.net/AWS 자격증 훈련 사이트/index.html)
- 프로젝트 설명: 시험 문제 풀이를 통한 학습과 미니 퀴즈를 통한 개념정리

<br>

## 📫 연락처(Contact)
- Email: csjang94@gmail.com
- Blog: [[블로그](https://csjang94-dev.tistory.com/)]







