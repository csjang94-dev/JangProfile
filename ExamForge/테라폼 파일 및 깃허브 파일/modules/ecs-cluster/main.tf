...
resource "aws_security_group" "ecs_service" {
  ...
  # 💡 [수정] 인그레스 소스를 ALB 보안 그룹으로 정확히 지정
  ingress {
...
resource "aws_lb" "main" {
  ...
  security_groups    = [aws_security_group.alb.id]
  subnets            = var.public_subnet_ids # Public Subnet에 배포
...
resource "aws_ecs_task_definition" "app" {
  ...
  task_role_arn      = var.ecs_task_role_arn # 애플리케이션 권한 (DynamoDB 등)
  execution_role_arn = aws_iam_role.ecs_task_execution_role.arn # ECS/ECR/CloudWatch 권한
...
resource "aws_ecs_service" "app" {
  ...
  # 💡 [수정] Task Definition 변경 시 서비스가 업데이트되도록 lifecycle ignore_changes 제거
